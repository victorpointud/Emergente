...
STATE_MLP = {"sizes": None, "layers": None}
...
def act_scalar(x, kind):
    k = (kind or "").lower()
    if k in ("sigmoid","sigmoide"): return sigmoid(x)
    if k == "tanh": return tanh(x)
    if k == "relu": return relu(x)
    if k in ("step","escalon"): return 1.0 if x>=0 else 0.0
    raise ValueError("Unsupported activation")

def dact_scalar(y, kind):
    k = (kind or "").lower()
    if k in ("sigmoid","sigmoide"): return dsigmoid(y)
    if k == "tanh": return dtanh(y)
    if k == "relu": return drelu(y)
    if k in ("step","escalon"): return 0.0
    raise ValueError("Unsupported activation")
...
def mse(y, t):
    s = 0.0
    for i in range(len(y)): s += (y[i]-t[i])**2
    return s/len(y)
...
def backprop_step(x, t, sizes, layers, act_hidden, act_out, lr):
    L = len(sizes)-1
    a0 = x[:]
    y, a_list, z_list = forward_full(x, sizes, layers, act_hidden, act_out)
    deltas = [None]*L
...
def eval_dataset(X, sizes, layers, act_hidden, act_out):
    Y = [forward(x,sizes,layers,act_hidden,act_out) for x in X]
    return Y
    ...
    def do_POST(self):
        ...
        if self.path == "/api/mlp/apply_config":
            try:
                ...
                if not isinstance(sizes, list) or len(sizes)<2: raise ValueError("Invalid sizes")
                if not isinstance(layers, list) or len(layers)!=len(sizes)-1: raise ValueError("Invalid layers")
                for l in range(len(layers)):
                    out_size = int(sizes[l+1]); in_size = int(sizes[l])
                    W = [float(v) for v in layers[l]["W"]]
                    b = [float(v) for v in layers[l]["b"]]
                    if len(W)!=out_size*in_size: raise ValueError(f"Layer {l}: invalid W shape")
                    if len(b)!=out_size: raise ValueError(f"Layer {l}: invalid b shape")
                    layers[l] = {"W":W,"b":b}
        ...
        if self.path == "/api/mlp/create":
            try:
                ...
                if isinstance(hidden_neurons, list):
                    if len(hidden_neurons)!=n_hidden: raise ValueError("hidden_neurons length mismatch")
        ...
        if self.path == "/api/mlp/train":
            try:
                if STATE_MLP["sizes"] is None or STATE_MLP["layers"] is None: raise ValueError("Configuration not applied")
                ...
                sizes = STATE_MLP["sizes"]; layers = STATE_MLP["layers"]
                hist = []
        ...
        if self.path == "/api/mlp/eval_one":
            try:
                ...
                act_hidden = data.get("act_hidden","sigmoid")
                act_out = data.get("act_out","sigmoid")
                if len(x)!=STATE_MLP["sizes"][0]: raise ValueError("Input size mismatch")
        ...
        if self.path == "/api/mlp/eval_file":
            try:
                ...
                for i,row in enumerate(rows, start=1):
                    try:
                        if len(row)!=STATE_MLP["sizes"][0]: raise ValueError("Input size mismatch")
